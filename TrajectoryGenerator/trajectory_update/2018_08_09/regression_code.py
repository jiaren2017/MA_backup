...
def stdLinReg (dataMat):                    # format the dataset into the target variable Y and the independent variable X
    ...
    #ws = np.linalg.pinv(xTx) * (X.T * Y)                          # calculate the optimal weight matrix ws with least-squares method
    return ws,X,Y
...
def cal_MSE(yArr_actual, yArr_pred):                          # calculate the squared error
    return (((yArr_actual - yArr_pred) **2).sum()) / yArr_actual.shape[0]
...
def localWeightLinReg(testPoint, dataMat_train, k = 1.0):       # run lwlr-prediction  on a SINGLE value
    ...
    if np.linalg.det(xTx) == 0.0:                                     # test the inversability
        ...
        return
    ws = xTx.I * (XMat_train.T * (weights * YMat_train))               # calculate the weights for "Locally weighted linear regression"
    #ws = np.linalg.pinv(xTx) * (XMat_train.T * (weights * YMat_train))
    return testPoint * ws   
...
def linearSolve(dataMat):  # format the dataset into the target variable Y and the independent variable X
    ...
    xTx = X.T*X
    if np.linalg.det(xTx) == 0.0:                   # check the inversability
        print("calculate ws = np.linalg.pinv(xTx)* (X.T * Y):  BEGIN")
        #raise NameError('This matrix is singular, cannot do inverse,try increasing the second value of Stop Condition')
        ws = np.linalg.pinv(xTx)* (X.T * Y)              # use np.linalg.pinv() to inverse the matrix
        print("calculate ws = np.linalg.pinv(xTx)* (X.T * Y):  END")
    else:
        print("calculate ws = xTx.I * (X.T * Y):  BEGIN")
        ws = xTx.I * (X.T * Y)                          # calculate the optimal weight matrix ws with least-squares method
        print("calculate ws = xTx.I * (X.T * Y):  END")
#    print("check pinv()")
    return ws,X,Y
...
def chooseBestSplit(dataMat, leafType = regLeaf, errType = regErr, stopCond = (1, 4), learnRate = 0.1, epoch = 50, minPercent = 0.01):
    ...
    for featIndex in range(n - 1):   # iterate all feature columns to find the splitting feature and splitting value
        for splitVal in set(dataMat[:,featIndex].T.tolist()[0]):    # iterate all x-values of ONE certain feature
            t = t + 1
            print("feature: ", featIndex, " and splitVal:  ", splitVal, " and t: ", t)
...
def createTree(dataMat_train, leafType = regLeaf, errType = regErr, stopCond = (1, 4), learnRate = 0.1, epoch = 50, minPercent = 0.01):
    ...
    if feat == None: 
        print("feat == None, return value")
        return val        # If stop condition met, return leaf value for the leaf node 
...
##########################   functions for save the tree to file    ##############################
...
def createForeCast_v2(tree_trained, dataMat_test, modelEval=regTreeEval, stepPred= False, treeX_trained=None, treeY_trained=None, numSteps=10):
    if (stepPred == False):                      #  just run forecast based on the test data, static
        m = len(dataMat_test)
        yArr_pred = np.zeros(m)
        for i in range(m):                        #  run prediction for each SINGLE value of test set
        ...
        # features are column vectors  and samples are row vectors (with n steps!)!!!
        dataMat_pred = np.mat(np.zeros((numSteps+1, dataMat_test.shape[1]+2)))  
        dataMat_pred[0, 0:-2] = dataMat_test        # assign the position and speed of test data
         
        for i in range(numSteps):
            ## copy the old steps to the next column 
            dataMat_pred[i+1, 0:-6] = dataMat_pred[i, 4:-2]
            
            
            # calculate the acceleratex x(i), y(i)
            dataMat_pred[i,-2] = treeForecast(treeX_trained, dataMat_pred[i,0:-2], modelEval)
            dataMat_pred[i,-1] = treeForecast(treeY_trained, dataMat_pred[i,0:-2], modelEval)
            
            # calculate the NEW position x(i+1), y(i+1)
            dataMat_pred[i+1,-6] = dataMat_pred[i,-6] + dataMat_pred[i,-4] + dataMat_pred[i,-2]
            dataMat_pred[i+1,-5] = dataMat_pred[i,-5] + dataMat_pred[i,-3] + dataMat_pred[i,-1]
            
            # calculate the NEW speed x(i+1), y(i+1)
            dataMat_pred[i+1,-4] = dataMat_pred[i,-4] + dataMat_pred[i,-2]
            dataMat_pred[i+1,-3] = dataMat_pred[i,-3] + dataMat_pred[i,-1]
        
        dataMat_pred[-1,-2] = treeForecast(treeX_trained, dataMat_pred[-1,0:-2], modelEval)
        dataMat_pred[-1,-1] = treeForecast(treeY_trained, dataMat_pred[-1,0:-2], modelEval)
