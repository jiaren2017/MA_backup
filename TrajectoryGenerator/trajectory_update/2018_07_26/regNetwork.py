...
def L_model_forward(X, parameters, regMode = True, foreAct = 'relu'):
    """
...
def compute_cost(AL, Y, regMode = True):
...
def L_model_backward(AL, Y, caches, regMode = True, backAct = 'relu'):
    """
...
######     complete model    #####
##################################################
def L_layer_model(X, Y, layers_dims, optimizer, learning_rate = 0.0075, num_iterations = 500, enable_mini_batch = False, mini_batch_size = 64, print_cost= False, plot_cost = False, regMode = True,
    ...
    for i in range(num_iterations):
        if (enable_mini_batch == True):
            ...
            for minibatch in minibatches:
                ...
                # Compute cost.
                cost = compute_cost(AL, minibatch_Y, regMode)
            
                # Backward propagation.
                grads = L_model_backward(AL, minibatch_Y, caches, regMode, backAct)
...
def network_predict_v2(dataMat_test, parameters_NN, norm_parameters_NN = None, regMode = True, foreAct = 'relu', stepPred= False, parameters_xNN=None, x_norm_parameters=None,  parameters_yNN=None, y_norm_parameters=None, numSteps=10):
    if (stepPred == False):                      #  just run forecast based on the test data, static
        ...
        dataMat_pred = np.mat(np.zeros((dataMat_test.shape[0]+2, numSteps)))   # features are rows vectors (with n steps!) and samples are column vectors !!!
