...
dataMat_test, dataMat_x_test, dataMat_y_test = read.read_hdf5_auto(test_path, test_Filename)
...
regNN_1234_x_file = 'regNN_1234_x_trained_3.dat'; regNN_parameters_x = regNN.loadNNetwork(regNN_1234_x_file)
regNN_1234_y_file = 'regNN_1234_y_trained_3.dat'; regNN_parameters_y = regNN.loadNNetwork(regNN_1234_y_file)
...
parameters_bike = regNN.loadNNetwork(regression_network_file)
...
### evaulate bike data
'''
yArr_pred_test = reg.createForeCast(modTree_bike, dataMat_test_bike[:,0], reg.modelTreeEval)
...
############################################################################
### evaulate walk trace data



### predict with function "L_model_forward"
#prediction_train_accX, caches_x = regNN.L_model_forward(train_x[:,:10], regNN_parameters_x, regMode = True, foreAct = 'relu')
#prediction_train_accX = np.array(prediction_train_accX).ravel()




### predict with function "network_predict"
...
dataMat_test = dataMat_org[random_pos,0:4]    # make the starting point as test data
#dataMat_test[:,2:] = 0                     # define the speed

## customize the test data
#dataMat_test = np.mat(np.array([300,300,0,-10]))
...
## predict!
dataMat_regNN_Pred = regNN.network_predict(dataMat_test, None, regMode = True, foreAct = 'relu', stepPred= True, 
...
reg.showStdLinReg(dataList_test, predictions_test.T)
'''
